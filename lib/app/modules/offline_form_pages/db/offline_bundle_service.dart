...
class OfflineBundleService {
  ...
  static Future<void> backupCurrentDatabase() async {
    ...
    try {
      ...
      final user = StorageService.userName ?? "Unknown";
  ...
  static Future<File?> createExportBundle() async {
    try {
      ...
      final String user = StorageService.userName ?? "UnknownUser";
    ...
    try {
      ...
      for (final file in archive) {
        if (file.name == 'offline_forms.db') {
          ...
          final allRecords = await tempDb.query(
          ...
          if (oldPathToReplace != null) {
            ...
            await _remapDatabasePaths(
              tempDb,
              oldPathToReplace,
              currentAppFlutterPath,
            );
          }

          await tempDb.close();

          final finalDbPath = join(
  ...
  static Future<void> importDbOnly(File dbFile) async {
    try {
      ...
      // LogService.writeLog(
      //   message: "$logTag [IMPORT_DB_ONLY_SUCCESS] DB replaced successfully.",
      // );
    } catch (e) {
  ...
  static Future<void> uploadDBFile() async {
    ...
    //               "payload": "",
    //               "axpfile_file": {
    //                 "file1": {"filename": fileName, "fileasbase64": base64Zip},
    //               },
    //             },
    //           },
    //         },
    ...
    //   final String url = AppConst.getFullARMUrl(
    ...
    try {
      ...
      for (final file in archive) {
        ...
        if (normalizedName == 'offline_forms.db') {
          ...
          // LogService.writeLog(
          //   message: "$logTag [REMAP] Starting exact file remapping in Dart...",
          // );
          final allRecords = await tempDb.query(
            OfflineDBConstants.TABLE_PENDING_REQUESTS,
          );
          ...
          // LogService.writeLog(
          //   message: "$logTag [IMPORT_SUCCESS] DB replaced and re-initialized.",
          // );
        }
      }
    } catch (e) {
