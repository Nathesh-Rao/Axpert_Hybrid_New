...
class OfflineBackgroundSyncService {
  ...
  Future<void> start() async {
    ...
    final bool autoSyncEnabled = StorageService.autoSync ?? false;
    if (!autoSyncEnabled) {
    ...
    if (!await _checkInternet()) {
      LogService.writeLog(message: '$_tag start() aborted — no internet.');
      return;
    }
    ...
    if (interval == 0) {
      log(
        '$_tag sync_interval_minutes=0 — sync disabled by server config.',
        name: _tag,
      );
      LogService.writeLog(
    ...
    final String armUrl = await AppConst.getFullARMUrl(
  ...
  Future<void> stop() async {
    await FlutterForegroundTask.stopService();
    _reset();
    LogService.writeLog(message: '$_tag Stopped.');
  ...
  Future<bool> setAutoSync({required bool enabled}) async {
    ...
    final bool current = StorageService.autoSync ?? false;
    if (current != enabled) {
  ...
  Future<void> setIntervalMinutes(int minutes) async {
    ...
    if (_isRunning) {
      ...
      LogService.writeLog(message: '$_tag Interval updated to $clamped min.');
    }
  }
    ...
    Get.dialog(
      WillPopScope(
        ...
        child: AlertDialog(
          ...
          actions: [
            TextButton(onPressed: () => Get.back(), child: const Text('Wait')),
            // TextButton(
            //   onPressed: () async {
            //     Get.back();
            //     await stop();
  ...
  Future<void> _saveIntervalToStorage(int minutes) async {
    try {
      ...
      final String? projectName = StorageService.projectName;
      if (username == null || projectName == null) return;
      StorageService.setString(
        _intervalStorageKey(username, projectName),
        minutes.toString(),
      );
    } catch (_) {}
  }
