...
class OfflineSyncTaskHandler extends TaskHandler {
  ...
  Future<void> onDestroy(DateTime timestamp, bool isTimeout) async {
    _timer?.cancel();
    _timer = null;
    LogService.writeLog(message: '$_tag onDestroy. isTimeout=$isTimeout');
  }
  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(Duration(minutes: _intervalMinutes), (_) async {
      if (!_isCycleRunning) {
        LogService.writeLog(message: '$_tag Timer fired — running sync cycle.');
        await _runSyncCycle();
      } else {
        LogService.writeLog(
  ...
  Future<void> _runSyncCycle() async {
    ...
    try {
      ...
      if (!await _checkInternet()) {
        LogService.writeLog(message: '$_tag No internet — skipping cycle.');
        _updateNotification(
          title: '📡 Sync Waiting',
      ...
      LogService.writeLog(message: '$_tag Push done. Result: $result');
      ...
      LogService.writeLog(message: '$_tag Cycle FAILED: $e');
      _updateNotification(
        title: '🔴 Sync Error',
  ...
  Future<void> _ensureDbInitialized() async {
    ...
    _dbInitialized = true;
    LogService.writeLog(message: '$_tag DB initialized in isolate.');
  }
